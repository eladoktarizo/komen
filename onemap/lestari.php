        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="4" required>Perdesaan Lestari<br>
					<input type="radio" name="status" value="1" required>Bukan Perdesaan Lestari<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO lestari (propinsi, kab_kota, lestari3,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE lestari SET propinsi='$propnew', kab_kota='$kota', lestari3='$status' ,tahun='$tahun' WHERE lestari_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from lestari where lestari_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM lestari where lestari_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from lestari where lestari_pk = '$id_edit'";
					$result = pg_query($qedit);
					while ($redit = pg_fetch_array($result)) {
						$propinsi = $redit['propinsi'];
						$kab = $redit['kab_kota'];
						$status = $redit['lestari3'];
						$id = $redit['lestari_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Perdesaan Lestari<br>
											  <input type="radio" name="status" value="1" required>Bukan Perdesaan Lestari<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Kota Perdesaan Lestari<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan Perdesaan Lestari<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM lestari ORDER BY lestari_pk DESC";
			...
			if ($tcount == 0) {
				...
				$status = $rowview['lestari3'];
				if ($status == 4){
					$status = "Perdesaan Lestari";
				}else{
					$status = "Bukan Perdesaan Lestari";
				}
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=lestari.php?id_edit=$rowview[lestari_pk]>Edit</a> | <a href=\"lestari.php?id_delete=$rowview[lestari_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
