        ...
        <div class="right col">
			...
			<table width="500" border="0">
			  ...
			  <tr>
				<td>SPAM Eksis</td>
				<td>:</td>
				<td><input type="text" name="spam" class="k-textbox"/>
				</td>
			  </tr>
			  <tr>
				<td>Skor</td>
				<td>:</td>
				<td><input type="text" name="skor" class="k-textbox"/>
				</td>
			  </tr>
			  <tr>
				<td>Tahun</td>
			  ...
			  <tr>
				...
				<td><input type="radio" name="status" value="1"/>Rawan Kering<br/><input type="radio" name="status" value="2"/>Potensi Kering<br/><input type="radio" name="status" value="3"/>Aman
				</td>
			  </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					if($status == 1){
						$keterangan = "Rawan Kering";
					}elseif($status == 2){
						$keterangan = "Potensi Kering";
					}else{
						$keterangan = "Aman";
					}
					...
					$qinput = "INSERT INTO rawanair (propinsi, kab_kota, kecamatan, desa, spam_eksis, skor, keterangan, status,tahun) VALUES ('$propnew', '$kota', '$kecamatan', '$desa', '$spam', '$skor', '$keterangan', '$status','$tahun');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE rawanair SET propinsi='$propnew', kab_kota='$kota', kecamatan='$kecamatan', desa='$desa', spam_eksis='$spam', skor='$skor', keterangan='$keterangan', status='$status' WHERE rawanair_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from rawanair where rawanair_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM rawanair where rawanair_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from rawanair where rawanair_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						$desa = $redit['desa'];
						$spam = $redit['spam_eksis'];
						$skor = $redit['skor'];
						$keterangan = $redit['keterangan'];
						$status = $redit['status'];
						$id = $redit['rawanair_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<tr>
								<td>SPAM Eksis</td>
								<td>:</td>
								<td><input type="text" name="spam" class="k-textbox" value="<?php echo $spam; ?>"/>
								</td>
							  </tr>
							  <tr>
								<td>Skor</td>
								<td>:</td>
								<td><input type="text" name="skor" class="k-textbox" value="<?php echo $skor; ?>"/>
								</td>
							  </tr>
							  <tr>
								<td>Tahun</td>
							  ...
							  <tr>
								...
								<?php 
									if ($status == 1){ ?>
										<input type="radio" name="status" value="1" checked="checked"/>Rawan Kering<br/><input type="radio" name="status" value="2"/>Potensi Kering<br/><input type="radio" name="status" value="3"/>Aman
								<?php
									}elseif($status == 2){ ?>
										<input type="radio" name="status" value="1" />Rawan Kering<br/><input type="radio" name="status" value="2" checked="checked"/>Potensi Kering<br/><input type="radio" name="status" value="3"/>Aman
								<?php
								}else{ ?>
									<input type="radio" name="status" value="1" />Rawan Kering<br/><input type="radio" name="status" value="2" />Potensi Kering<br/><input type="radio" name="status" value="3" checked="checked"/>Aman
								<?php
								}
								?>								
								</td>
			...
			$sqlview = "SELECT * FROM rawanair ORDER BY rawanair_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr style='font-size:13px;'>";
					...
					echo "<td>$rowview[spam_eksis]</td>";
					echo "<td>$rowview[skor]</td>";
					echo "<td>$rowview[keterangan]</td>";
					echo "<td>$rowview[tahun]</td>";
					echo "<td>$st</td>";
					echo "<td align=\"center\"><a href=rawanair.php?id_edit=$rowview[rawanair_pk]>Edit</a> | <a href=\"rawanair.php?id_delete=$rowview[rawanair_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
