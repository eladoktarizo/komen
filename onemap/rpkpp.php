        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                <td>Status</td>
                <td>:</td>
                <td><input type="radio" name="status" value="4" required>RPKPP<br>
					<input type="radio" name="status" value="1" required>Bukan RPKPP<br></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO rpkpp (propinsi, kab_kota, status,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE rpkpp SET propinsi='$propnew', kab_kota='$kota', status='$status',tahun='$tahun'  WHERE rpkpp_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from rpkpp where rpkpp_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM rpkpp where rpkpp_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from rpkpp where rpkpp_pk = '$id_edit'";
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
										if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">RPKPP<br>
											  <input type="radio" name="status" value="1" required>Bukan RPKPP<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >RPKPP<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan RPKPP<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM rpkpp ORDER BY rpkpp_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=rpkpp.php?id_edit=$rowview[rpkpp_pk]>Edit</a> | <a href=\"rpkpp.php?id_delete=$rowview[rpkpp_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
