        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                <td>Status</td>
                <td>:</td>
                <td><input type="radio" name="status" value="4" required>KSN<br>
					<input type="radio" name="status" value="1" required>Bukan KSN<br></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO ksn (propinsi, kab_kota, ksn1,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE ksn SET propinsi='$propnew', kab_kota='$kota', ksn1='$status',tahun='$tahun'  WHERE ksn_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from ksn where ksn_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM ksn where ksn_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from ksn where ksn_pk = '$id_edit'";
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">KSN<br>
											  <input type="radio" name="status" value="1" required>Tidak Termasuk KSN<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >KSN<br>
											  <input type="radio" name="status" value="1" required checked="checked">Tidak Termasuk KSN<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM ksn ORDER BY ksn_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=ksn.php?id_edit=$rowview[ksn_pk]>Edit</a> | <a href=\"ksn.php?id_delete=$rowview[ksn_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
