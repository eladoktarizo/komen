    ...
    <body background="img/bg.png">
        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                ...
                <td><!-- <input name="status" type="checkbox" class="k-checkbox" value="1" required />&nbsp;Ya <span style="font-size:9px; color:#FF0000;">(Wajib diberi check untuk validasi kabupaten)</span> --> 
					<input type="radio" name="status" value="4" required>Kota Hijau<br>
					<input type="radio" name="status" value="1" required>Bukan Kota Hijau<br>
				</td> 
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO hijau (propinsi, kab_kota, hijau3,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE hijau SET propinsi='$propnew', kab_kota='$kota', hijau3='$status',tahun='$tahun' WHERE hijau_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from hijau where hijau_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM hijau where hijau_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from hijau where hijau_pk = '$id_edit'";
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td><!--<input name="status" type="checkbox" class="k-checkbox" value="<?php echo $status;?>" required />&nbsp;Ya <span style="font-size:9px; color:#FF0000;">(Wajib diberi check untuk validasi kabupaten)</span>-->
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Kota Hijau<br>
											  <input type="radio" name="status" value="1" required>Bukan Kota Hijau<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Kota Hijau<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan Kota Hijau<br>';
									}
									?>
									<!-- <input type="radio" name="status" value="4" required checked="checked">Kota Hijau<br>
									<input type="radio" name="status" value="1" required>Bukan Kota Hijau<br>-->
								</td>
							  </tr>
							  <tr>
			...
			$sqlview = "SELECT * FROM hijau ORDER BY hijau_pk DESC";
			...
			if ($tcount == 0) {
				...
				$s = $i+1;
				$status = $rowview['hijau3'];
				if ($status == 4){
					$status = "Kota Hijau";
				}else{
					$status = "Bukan Kota Hijau";
				}
				echo "<tr >";
					echo "<td>$s</td>";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=hijau.php?id_edit=$rowview[hijau_pk]>Edit</a> | <a href=\"hijau.php?id_delete=$rowview[hijau_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
