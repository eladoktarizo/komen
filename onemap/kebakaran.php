        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="4" required>Kawasan Rawan Kebakaran<br>
					<input type="radio" name="status" value="1" required>Bukan Kws Rawan Kebakaran<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO kebakaran (propinsi, kab_kota, status,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE kebakaran SET propinsi='$propnew', kab_kota='$kota', status='$status',tahun='$tahun' WHERE kebakaran_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from kebakaran where kebakaran_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM kebakaran where kebakaran_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from kebakaran where kebakaran_pk = '$id_edit'";
					$result = pg_query($qedit);
					while ($redit = pg_fetch_array($result)) {
						$propinsi = $redit['propinsi'];
						$kab = $redit['kab_kota'];
						$status = $redit['status'];
						$id = $redit['kebakaran_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Kawasan Rawan Kebakaran<br>
											  <input type="radio" name="status" value="1" required>Bukan Kws Rawan Kebakaran<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Kawasan Rawan Kebakaran<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan Kws Rawan Kebakaran<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM kebakaran ORDER BY kebakaran_pk DESC";
			...
			if ($tcount == 0) {
				...
				if ($status == 4){
					$status = "Kawasan Rawan Kebakaran";
				}else{
					$status = "Bukan Kws Rawan Kebakaran";
				}
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=kebakaran.php?id_edit=$rowview[kebakaran_pk]>Edit</a> | <a href=\"kebakaran.php?id_delete=$rowview[kebakaran_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
