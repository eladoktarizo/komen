		...
		while($info = mysql_fetch_array( $check1 )){
			if ($pass1 != $info['passwd']){ 
				...
				if ($info['username'] == 'juri1') {//fasilitator
					$get = "select * from pertanyaan where tanggal = '$ayena' and level = '1' and status = '2' order by id asc";
				} else if ($info['username'] == 'juri2') {//participant genap
					$get = "select * from pertanyaan where tanggal = '$ayena' and level = '0' and (id%2=0) and status = '2' order by id asc";
				} else if ($info['username'] == 'juri3') {//participant ganjil
					$get = "select * from pertanyaan where tanggal = '$ayena' and level = '0' and (id%2=1) and status = '2' order by id asc";
				} else {
