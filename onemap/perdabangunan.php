 ...
 <html>
    <head>
        ...
        <style type="text/css">
            ...
            .header.row { height: 50px; line-height: 50px; background-color:#306fa9;}
            .body.row { top: 50px; bottom: 25px; }
            .footer.row { height: 25px; bottom: 0; line-height: 25px; font-size:10px; background-color:#306fa9;}
        </style>
    ...
    <body background="img/bg.png">
        <div class="left col">
            <div class="header row">
                <div class="judul">OneMap CRUD</div>
            </div>
        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="4" required>Sudah Memiliki Perda<br>
					<input type="radio" name="status" value="1" required>Belum Memiliki Perda<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO perdabangunan (propinsi, kab_kota, kawasan, status) VALUES ('$propnew', '$kota', '$tema', '$status');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE perdabangunan SET propinsi='$propnew', kab_kota='$kota', kawasan='$tema', status='$status' WHERE perdabangunan_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from perdabangunan where perdabangunan_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM perdabangunan where perdabangunan_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from perdabangunan where perdabangunan_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						?>
							...
							<tr>
								...
								<td><?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Sudah Memiliki Perda<br>
											  <input type="radio" name="status" value="1" required>Belum Memiliki Perda<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Sudah Memiliki Perda<br>
											  <input type="radio" name="status" value="1" required checked="checked">Belum Memiliki Perda<br>';
									}
									?>
								</td>
							</tr>
			...
			$sqlview = "SELECT * FROM perdabangunan ORDER BY perdabangunan_pk DESC";
			...
			if ($tcount == 0) {
				...
				if ($status == 4){
					$status = "Sudah Memiliki Perda";
				}else{
					$status = "Belum Memiliki Perda";
				}
				...
				echo "<tr style='font-size:12px;'>";
					...
					echo "<td>$rowview[kawasan]</td>";
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=perdabangunan.php?id_edit=$rowview[perdabangunan_pk]>Edit</a> | <a href=\"perdabangunan.php?id_delete=$rowview[perdabangunan_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
