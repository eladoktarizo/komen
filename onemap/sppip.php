        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                <td>Status</td>
                <td>:</td>
                <td><input type="radio" name="status" value="4" required>SPPIP<br>
					<input type="radio" name="status" value="1" required>Bukan SPPIP<br></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO sppip (propinsi, kab_kota, status,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE sppip SET propinsi='$propnew', kab_kota='$kota', status='$status' ,tahun='$tahun' WHERE sppip_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from sppip where sppip_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM sppip where sppip_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from sppip where sppip_pk = '$id_edit'";
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">SPPIP<br>
											  <input type="radio" name="status" value="1" required>Tidak Termasuk SPPIP<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >SPPIP<br>
											  <input type="radio" name="status" value="1" required checked="checked">Tidak Termasuk SPPIP<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM sppip ORDER BY sppip_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=sppip.php?id_edit=$rowview[sppip_pk]>Edit</a> | <a href=\"sppip.php?id_delete=$rowview[sppip_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
