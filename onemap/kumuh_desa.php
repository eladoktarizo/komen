        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                <td>Status</td>
                <td>:</td>
                <td><input name="status" type="checkbox" class="k-checkbox" value="1" required />&nbsp;Ya <span style="font-size:9px; color:#FF0000;">(Wajib diberi check untuk validasi kabupaten)</span></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO kumuh_desa (propinsi, kab_kota, kecamatan, desa, tema_penan, status,tahun) VALUES ('$propnew', '$kota', '$kecamatan', '$desa', '$tema', '$status','$tahun');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE kumuh_desa SET propinsi='$propnew', kab_kota='$kota', kecamatan='$kecamatan', desa='$desa', tema_penan='$tema', status='$status' ,tahun='$tahun' WHERE kumuh_desa_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from kumuh_desa where kumuh_desa_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM kumuh_desa where kumuh_desa_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from kumuh_desa where kumuh_desa_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						?>
							...
							<tr>
								<td>Status</td>
								<td>:</td>
								<td><input name="status" type="checkbox" class="k-checkbox" value="<?php echo $status;?>" required />&nbsp;Ya <span style="font-size:9px; color:#FF0000;">(Wajib diberi check untuk validasi kabupaten)</span></td>
							</tr>
							  <tr>
			...
			$sqlview = "SELECT * FROM kumuh_desa ORDER BY kumuh_desa_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr style='font-size:13px;'>";
					...
					echo "<td align=\"center\"><a href=kumuh_desa.php?id_edit=$rowview[kumuh_desa_pk]>Edit</a> | <a href=\"kumuh_desa.php?id_delete=$rowview[kumuh_desa_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
