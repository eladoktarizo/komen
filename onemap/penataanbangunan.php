        ...
        <div class="right col">
			...
			<table width="500" border="0">
			  ...
			  <tr>
				<td>Kawasan</td>
				<td>:</td>
				<td><input type="text" name="kawasan" class="k-textbox"/>
				</td>
			  </tr>
			  <tr>
				<td>Tahun</td>
			  ...
			  <tr>
				...
				<td>:</td>
				<td>
					<input type="radio" name="status" value="1" required>RTBL<br>
					<input type="radio" name="status" value="3" required>Tidak Termasuk RTBL<br>
				</td>
			  </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO penataanbangunan (propinsi, kab_kota, kawasan, status,tahun) VALUES ('$propnew', '$kota'	, '$kawasan', '$status','$tahun');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE penataanbangunan SET propinsi='$propnew', kab_kota='$kota', kawasan='$kawasan', status='$status',tahun='$tahun'  WHERE penataanbangunan_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from penataanbangunan where penataanbangunan_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM penataanbangunan where penataanbangunan_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from penataanbangunan where penataanbangunan_pk = '$id_edit'";
					$result = pg_query($qedit);
					while ($redit = pg_fetch_array($result)) {
						$propinsi = $redit['propinsi'];
						$kab = $redit['kab_kota'];
						$kawasan = $redit['kawasan'];
						$status = $redit['status'];
						$id = $redit['penataanbangunan_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<table width="500" border="0">
							  <tr>
								...
								<tr>
									<td>Nama Kawasan</td>
									<td>:</td>
									<td><input type="text" name="kawasan" class="k-textbox" value="<?php echo $kawasan;?>"/>
									</td>	
								</tr>
							  <tr>
								<td>Tahun</td>
								...
								<tr>
									...
									<td>:</td>
									<?php
									if($status=="1" || $status=="RTBL"){
									
									?>
									<td>
										<input type="radio" name="status" value="1" required checked>RTBL<br>
										<input type="radio" name="status" value="3" required>Tidak Termasuk RTBL<br>
									</td>	
									<?
									}else{
									...
									<td>
										<input type="radio" name="status" value="1" required>RTBL<br>
										<input type="radio" name="status" value="3" required checked>Tidak Termasuk RTBL<br>
									</td>	
									<?}?>	
									<!--td>:</td>
									<td><input type="number" min="1" max="3" name="status" class="k-textbox" value="<?php echo $status;?>"/>
									</td-->	
								</tr>
								  <tr>
			...
			$sqlview = "SELECT * FROM penataanbangunan ORDER BY penataanbangunan_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr style='font-size:13px;'>";
					...
					echo "<td>$st</td>";
					echo "<td align=\"center\"><a href=penataanbangunan.php?id_edit=$rowview[penataanbangunan_pk]>Edit</a> | <a href=\"penataanbangunan.php?id_delete=$rowview[penataanbangunan_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
