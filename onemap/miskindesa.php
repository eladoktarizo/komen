        ...
        <div class="right col">
			...
			<table width="500" border="0">
			  ...
			  <tr>
                <td>Kegiatan</td>
                <td>:</td>
                <td><input type="text" name="kegiatan" class="k-textbox"/>
                </td>
              </tr>
			  <tr>
				<td>Tahun</td>
              ...
              <tr>
                <td>Status</td>
                <td>:</td>
                <td><!-- <input name="status" type="checkbox" class="k-checkbox" value="1" required />&nbsp;Ya <span style="font-size:9px; color:#FF0000;">(Wajib diberi check untuk validasi kabupaten)</span> -->
					<input type="radio" name="status" value="4" required>Miskin Desa<br>
					<input type="radio" name="status" value="1" required>Bukan Miskin Desa<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO miskindesa (propinsi, kab_kota, kecamatan, desa, kegiatan, status,tahun) VALUES ('$propnew', '$kota', '$kecamatan', '$desa', '$kegiatan', '$status','$tahun');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE miskindesa SET propinsi='$propnew', kab_kota='$kota', kecamatan='$kecamatan', desa='$desa', kegiatan='$kegiatan', status='$status' ,tahun='$tahun' WHERE miskindesa_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from miskindesa where miskindesa_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM miskindesa where miskindesa_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from miskindesa where miskindesa_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						$desa = $redit['desa'];
						$kegiatan = $redit['kegiatan'];
						$status = $redit['status'];
						$id = $redit['miskindesa_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<tr>
								<td>Kegiatan</td>
								<td>:</td>
								<td><input type="text" name="kegiatan" class="k-textbox" value="<?php echo $kegiatan;?>"/>
								</td>
							</tr>
							  <tr>
								<td>Tahun</td>
							...
							<tr>
								<td>Status</td>
								<td>:</td>
								<td><!--<input name="status" type="checkbox" class="k-checkbox" value="<?php //echo $status;?>" required />&nbsp;Ya <span style="font-size:9px; color:#FF0000;">(Wajib diberi check untuk validasi kabupaten)</span>-->
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Miskin Desa<br>
											  <input type="radio" name="status" value="1" required>Bukan Miskin Desa<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Miskin Desa<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan Miskin Desa<br>';
									}
									?>
								</td>
							</tr>
			...
			$sqlview = "SELECT * FROM miskindesa ORDER BY miskindesa_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr style='font-size:13px;'>";
					...
					echo "<td>$rowview[kegiatan]</td>";
					echo "<td>$rowview[tahun]</td>";
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=miskindesa.php?id_edit=$rowview[miskindesa_pk]>Edit</a> | <a href=\"miskindesa.php?id_delete=$rowview[miskindesa_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
