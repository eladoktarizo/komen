...
for (var i=0, n=checkboxes.length;i<n;i++) {
  ...
  var lvl = $('#lvl'+idna).val();
		...
		$.jsonp({
			url: "http://ciptakarya.pu.go.id/comment/accept.php?id="+idna+"&nama="+nama+"&direktorat="+direk+"&pembw="+pembawa+"&komen="+komeng+"&st="+status+"&vt=0&idus="+idu+"&level="+lvl,
			callback: "callback",
			success: function(data) {
				...
				$.each(data, function(i, item){
					...
					var tt = item.tt;
					if (tt == 1) {
						$.post( "upload_pr.php",{idna:id}, function( data ) {
