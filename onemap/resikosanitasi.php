        ...
        <div class="right col">
			...
			<table width="500" border="0">
			  ...
			  <tr>
                <td>Area Beresiko</td>
                <td>:</td>
                <td><input type="text" name="areaberesiko" class="k-textbox"/>
                </td>
              </tr>
			  <tr>
                <td>Penyebab</td>
                <td>:</td>
                <td><input type="text" name="penyebab" class="k-textbox"/>
                </td>
              </tr>
			  <tr>
				<td>Tahun</td>
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="4" required>Kurang Beresiko<br>
					<input type="radio" name="status" value="3" required>Resiko Sedang<br>
					<input type="radio" name="status" value="2" required>Resiko Tinggi<br>
					<input type="radio" name="status" value="1" required>Resiko Sangat Tinggi<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					if(isset($_POST["areaberesiko"])){
							$areaberesiko = $_POST["areaberesiko"];
						}
					if(isset($_POST["penyebab"])){
					...
					$qinput = "INSERT INTO resikosanitasi (propinsi, kab_kota, kecamatan, desa, areaberesi, penyebab, status,tahun) VALUES ('$propnew', '$kota', '$kecamatan', '$desa', '$areaberesiko', '$penyebab', '$status','$tahun');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE resikosanitasi SET propinsi='$propnew', kab_kota='$kota', kecamatan='$kecamatan', desa='$desa', areaberesi='$areaberesiko', penyebab='$penyebab', status='$status' ,tahun='$tahun' WHERE resikosanitasi_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from resikosanitasi where resikosanitasi_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM resikosanitasi where resikosanitasi_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from resikosanitasi where resikosanitasi_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						$desa = $redit['desa'];
						$areaberesiko = $redit['areaberesi'];
						$penyebab = $redit['penyebab'];
						$status = $redit['status'];
						$id = $redit['resikosanitasi_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<tr>
								<td>Area Beresiko</td>
								<td>:</td>
								<td><input type="text" name="areaberesiko" class="k-textbox" value="<?php echo $areaberesiko;?>"/>
								</td>
							</tr>
							<tr>
								<td>Penyebab</td>
								<td>:</td>
								<td><input type="text" name="penyebab" class="k-textbox" value="<?php echo $penyebab;?>"/>
								</td>
							</tr>
							  <tr>
								<td>Tahun</td>
							...
							<tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Kurang Beresiko<br>
											  <input type="radio" name="status" value="3" required>Resiko Sedang<br>
											  <input type="radio" name="status" value="2" required>Resiko Tinggi<br>
											  <input type="radio" name="status" value="1" required>Resiko Sangat Tinggi<br>';
									}else if ($status == 3){
										echo '<input type="radio" name="status" value="4" required>Kurang Beresiko<br>
											  <input type="radio" name="status" value="3" required checked="checked">Resiko Sedang<br>
											  <input type="radio" name="status" value="2" required>Resiko Tinggi<br>
											  <input type="radio" name="status" value="1" required>Resiko Sangat Tinggi<br>';
									}else if ($status == 2){
										echo '<input type="radio" name="status" value="4" required>Kurang Beresiko<br>
											  <input type="radio" name="status" value="3" required>Resiko Sedang<br>
											  <input type="radio" name="status" value="2" required checked="checked">Resiko Tinggi<br>
											  <input type="radio" name="status" value="1" required>Resiko Sangat Tinggi<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required>Kurang Beresiko<br>
											  <input type="radio" name="status" value="3" required>Resiko Sedang<br>
											  <input type="radio" name="status" value="2" required>Resiko Tinggi<br>
											  <input type="radio" name="status" value="1" required checked="checked">Resiko Sangat Tinggi<br>';
									}		  
									?>
								</td>
							</tr>
			...
			$sqlview = "SELECT * FROM resikosanitasi ORDER BY resikosanitasi_pk DESC";
			...
			if ($tcount == 0) {
				...
				if ($status == 4){
					$status = "Kurang Beresiko";
				}else if ($status == 3){
					$status = "Resiko Sedang";
				}else if ($status == 2){
					$status = "Resiko Tinggi";
				}else{
					$status = "Resiko Sangat Tinggi";
				}
				...
				echo "<tr style='font-size:11px;'>";
					...
					echo "<td>$rowview[areaberesi]</td>";
					echo "<td>$rowview[penyebab]</td>";
					echo "<td>$rowview[tahun]</td>";
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=resikosanitasi.php?id_edit=$rowview[resikosanitasi_pk]>Edit</a> | <a href=\"resikosanitasi.php?id_delete=$rowview[resikosanitasi_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
