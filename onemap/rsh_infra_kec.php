    ...
    <body background="img/bg.png">
        ...
        <div class="right col">
			...
			<table width="500" border="0">
			  ...
			  <tr>
				...
				<tr>
					<td>Nama Kawasan</td>
					<td>:</td>
					<td><input type="text" name="namakawasan" class="k-textbox"/>
					</td>	
				</tr>
				<tr>
					<td>Tahun</td>
			...
			<?php
					...
					if(isset($_POST["namakawasan"])){
							$namakawasan = $_POST["namakawasan"];
						}
					...
					$qinput = "INSERT INTO rsh_infra_kec (propinsi, kab_kota, kec, nama_kawasan, tahun, jumlah, status) VALUES ('$propnew', '$kota', '$kecamatan', '$namakawasan', '$tahun', '$jumlah', '$status');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE rsh_infra_kec SET propinsi='$propnew', kab_kota='$kota', kec='$kecamatan', nama_kawasan='$namakawasan', jumlah='$jumlah', tahun='$tahun' WHERE rsh_infra_kec_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from rsh_infra_kec where rsh_infra_kec_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM rsh_infra_kec where rsh_infra_kec_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from rsh_infra_kec where rsh_infra_kec_pk = '$id_edit'";
					$result = pg_query($qedit);
					while ($redit = pg_fetch_array($result)) {
						$propinsi = $redit['propinsi'];
						$kab = $redit['kab_kota'];
						$kecamatan = $redit['kec'];
						$namakawasan = $redit['nama_kawasan'];
						$tahun = $redit['tahun'];
						$jumlah = $redit['jumlah'];
						$id = $redit['rsh_infra_kec_pk'];
						?>
							...
							<tr>
								...
								<td>:</td>
								<td><input type="text" name="namakawasan" class="k-textbox" value="<?php echo $namakawasan;?>"/>
								</td>	
							</tr>
							<tr>
								<td>Tahun</td>
			...
			$sqlview = "SELECT * FROM rsh_infra_kec ORDER BY rsh_infra_kec_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr style='font-size:13px;'>";
					...
					echo "<td align=\"center\"><a href=rsh_infra_kec.php?id_edit=$rowview[rsh_infra_kec_pk]>Edit</a> | <a href=\"rsh_infra_kec.php?id_delete=$rowview[rsh_infra_kec_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
