        ...
        <div class="right col">
			...
			<table width="500" border="0">
			  ...
			  <tr>
                ...
                <td>:</td>
                <td>
					<input name="namakws" type="text" class="k-textbox"/>
				</td>
              </tr>
			  <tr>
                <td>Tema</td>
                <td>:</td>
                <td>
					<input name="tema" type="text" class="k-textbox"/>
				</td>
              </tr>
			  <tr>
                <td>Keterangan</td>
                <td>:</td>
                <td>
					<input name="keterangan" type="text" class="k-textbox"/>
				</td>
              </tr>
			  <tr>
				<td>Tahun</td>
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="1" required>Tidak Termasuk PPIP<br>
					<input type="radio" name="status" value="4" required>Sasaran PPIP<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					if(isset($_POST["namakws"])){
							$namakws = $_POST["namakws"];
						}	
					if(isset($_POST["tema"])){
							$tema = $_POST["tema"];
						}
					...
					$qinput = "INSERT INTO ppip (propinsi, kab_kota, kecamatan, desa, nama_kws, tema_pengn, keterangan, status, level,tahun) VALUES ('$propnew', '$kota', '$kecamatan', '$desa', '$namakws', '$tema', '$keterangan', '$status', '4','$tahun');";
					pg_query($qinput);
					//echo $qinput;
					...
					$qedit = "UPDATE ppip SET propinsi='$propnew', kab_kota='$kota', kecamatan='$kecamatan', desa='$desa', nama_kws='$namakws', tema_pengn='$tema', keterangan='$keterangan', status='$status', level='4',tahun='$tahun' WHERE ppip_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from ppip where ppip_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM ppip where ppip_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from ppip where ppip_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						$desa = $redit['desa'];
						$namakws = $redit['nama_kws'];
						$tema = $redit['tema_pengn'];
						...
						?>
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>:</td>
								<td>
									<input name="namakws" type="text" class="k-textbox" value="<?php echo $namakws;?>"/>
								</td>
							  </tr>
							  <tr>
								<td>Tema</td>
								<td>:</td>
								<td>
									<input name="tema" type="text" class="k-textbox" value="<?php echo $tema;?>"/>
								</td>
							  </tr>
							  <tr>
								<td>Keterangan</td>
								<td>:</td>
								<td>
									<input name="keterangan" type="text" class="k-textbox" value="<?php echo $keterangan;?>"/>
								</td>
							  </tr>
							  <tr>
								<td>Tahun</td>
							  ...
							  <tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="1" required>Tidak Termasuk PPIP<br>
											  <input type="radio" name="status" value="4" required checked="checked">Sasaran PPIP<br>';
									}else{
										echo '<input type="radio" name="status" value="1" required checked="checked">Tidak Termasuk PPIP<br>
											  <input type="radio" name="status" value="4" required>Sasaran PPIP<br>';
									}
									?>
								</td>
			...
			$sqlview = "SELECT * FROM ppip ORDER BY ppip_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr class='kecil' style='font-size:12px;'>";
					echo "<td>$s</td>";
					echo "<td>$rowview[propinsi]</td>"; 
					echo "<td>$rowview[kab_kota]</td>";
					echo "<td>$rowview[kecamatan]</td>";
					echo "<td>$rowview[desa]</td>";
					echo "<td>$rowview[nama_kws]</td>";
					echo "<td>$rowview[tema_pengn]</td>";
					echo "<td>$rowview[keterangan]</td>";					
					echo "<td>$rowview[tahun]</td>";					
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=ppip.php?id_edit=$rowview[ppip_pk]>Edit</a> | <a href=\"ppip.php?id_delete=$rowview[ppip_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
