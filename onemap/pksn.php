        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                <td>Status</td>
                <td>:</td>
                <td><input type="radio" name="status" value="4" required>PKSN<br>
					<input type="radio" name="status" value="1" required>Tidak Termasuk PKSN<br></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO pksn (propinsi, kab_kota, pksn1,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE pksn SET propinsi='$propnew', kab_kota='$kota', pksn1='$status' ,tahun='$tahun' WHERE pksn_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from pksn where pksn_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM pksn where pksn_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from pksn where pksn_pk = '$id_edit'";
						...
						?>
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td><?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">PKSN<br>
											  <input type="radio" name="status" value="1" required>Tidak Termasuk PKSN<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >PKSN<br>
											  <input type="radio" name="status" value="1" required checked="checked">Tidak Termasuk PKSN<br>';
									}
									?></td>
							  </tr>
							  <tr>
			...
			$sqlview = "SELECT * FROM pksn ORDER BY pksn_pk DESC";
			...
			if ($tcount == 0) {
				...
				$status = $rowview['pksn1'];
				if ($status == 4){
					$status = "PKSN";
				}else{
					$status = "Tidak Termasuk PKSN";
				}
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=pksn.php?id_edit=$rowview[pksn_pk]>Edit</a> | <a href=\"pksn.php?id_delete=$rowview[pksn_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
