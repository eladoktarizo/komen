        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="4" required>Bersejarah Tradisional<br>
					<input type="radio" name="status" value="1" required>Bukan Bersejarah Tradisional<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO bersejarahtradisional (propinsi, kab_kota, status,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE bersejarahtradisional SET propinsi='$propnew', kab_kota='$kota', status='$status' ,tahun='$tahun' WHERE bersejarahtradisional_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from bersejarahtradisional where bersejarahtradisional_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM bersejarahtradisional where bersejarahtradisional_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from bersejarahtradisional where bersejarahtradisional_pk = '$id_edit'";
					...
					while ($redit = pg_fetch_array($result)) {
						...
						$status = $redit['status'];
						$tahun = $redit['tahun'];
						$id = $redit['bersejarahtradisional_pk'];
						?>
							...
							<table width="100%" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
										if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Bersejarah Tradisional<br>
											  <input type="radio" name="status" value="1" required>Bukan Bersejarah Tradisional<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Bersejarah Tradisional<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan Bersejarah Tradisional<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM bersejarahtradisional ORDER BY bersejarahtradisional_pk DESC";
			...
			if ($tcount == 0) {
				...
				if ($status == 4){
					$status = "Bersejarah Tradisional";
				}else{
					$status = "Bukan Bersejarah";
				}
				echo "<tr >";
					echo "<td>$s</td>";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=bersejarahtradisional.php?id_edit=$rowview[bersejarahtradisional_pk]>Edit</a> | <a href=\"bersejarahtradisional.php?id_delete=$rowview[bersejarahtradisional_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
