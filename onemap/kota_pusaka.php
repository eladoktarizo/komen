        ...
        <div class="right col">
			...
			<table width="500" border="0">
              ...
              <tr>
                ...
                <td>:</td>
                <td>
					<input type="radio" name="status" value="4" required>Kota Pusaka<br>
					<input type="radio" name="status" value="1" required>Bukan Kota Pusaka<br>
				</td>
              </tr>
              <tr>
                <td></td>
			...
			<?php
					...
					$qinput = "INSERT INTO kota_pusaka (propinsi, kab_kota, status,tahun) VALUES ('$propnew', '$kota', '$status','$tahun');";
					pg_query($qinput);
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE kota_pusaka SET propinsi='$propnew', kab_kota='$kota', status='$status',tahun='$tahun' WHERE kota_pusaka_pk='$id'";
					pg_query($qedit);
					echo "<div class=\"success\">";
					...
					$query = "select * from kota_pusaka where kota_pusaka_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM kota_pusaka where kota_pusaka_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from kota_pusaka where kota_pusaka_pk = '$id_edit'";
					$result = pg_query($qedit);
					while ($redit = pg_fetch_array($result)) {
						$propinsi = $redit['propinsi'];
						$kab = $redit['kab_kota'];
						$status = $redit['status'];
						$id = $redit['kota_pusaka_pk'];
						$tahun = $redit['tahun'];
						?>
							...
							<table width="500" border="0">
							  ...
							  <tr>
								...
								<td>
									<?php
									if ($status == 4){
										echo '<input type="radio" name="status" value="4" required checked="checked">Kota Pusaka<br>
											  <input type="radio" name="status" value="1" required>Bukan Kota Pusaka<br>';
									}else{
										echo '<input type="radio" name="status" value="4" required >Kota Kota Pusaka<br>
											  <input type="radio" name="status" value="1" required checked="checked">Bukan Kota Pusaka<br>';
									}
									?>
								</td>
							  </tr>
			...
			$sqlview = "SELECT * FROM kota_pusaka ORDER BY kota_pusaka_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr >";
					...
					echo "<td>$status</td>";
					echo "<td align=\"center\"><a href=kota_pusaka.php?id_edit=$rowview[kota_pusaka_pk]>Edit</a> | <a href=\"kota_pusaka.php?id_delete=$rowview[kota_pusaka_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
