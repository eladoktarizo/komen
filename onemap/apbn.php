    ...
    <body background="img/bg.png">
        ...
        <div class="right col">
			...
			<?php
					...
					$qinput = "INSERT INTO apbn (provinsi, kab_kota, kecamatan, desa, tahun, status) VALUES ('$propnew', '$kota', '$kecamatan', '$desa', '$tahun', '$status');";
					pg_query($qinput);
					//echo $qinput;
					...
					$charini = 	array("+");
					$propnew = str_replace($charini, " ",$propinsi);
					$qedit = "UPDATE apbn SET provinsi='$propnew', kab_kota='$kota', kecamatan='$kecamatan', desa='$desa', tahun='$tahun', status='$status' WHERE apbn_pk='$id'";
					pg_query($qedit);
					//echo $qedit;
					echo "<div class=\"success\">";
					...
					$query = "select * from apbn where apbn_pk = '$id_delete'";
					...
					$sql2 = "DELETE FROM apbn where apbn_pk = '$id_delete'";
					pg_query($sql2);
				}else if(isset($_GET['id_edit'])){		//form edit data
					$id_edit = $_GET['id_edit'];
					$qedit = "select * from apbn where apbn_pk = '$id_edit'";
					$result = pg_query($qedit);
					while ($redit = pg_fetch_array($result)) {
						$propinsi = $redit['provinsi'];
						$kab = $redit['kab_kota'];
			...
			$sqlview = "SELECT * FROM apbn ORDER BY apbn_pk DESC";
			...
			if ($tcount == 0) {
				...
				echo "<tr style='font-size:13px;'>";
					...
					echo "<td align=\"center\"><a href=apbn.php?id_edit=$rowview[apbn_pk]>Edit</a> | <a href=\"apbn.php?id_delete=$rowview[apbn_pk]\" onclick='return confirmDelete();'>Delete</a></td>";
				echo "</tr>";
				$i++;
				$count++;
				}
